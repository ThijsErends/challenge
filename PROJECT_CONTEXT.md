# Project Context: The Labyrinth of the 4-Digit Goat
...
This is a high-level overview of the 10 puzzles. Each puzzle will have its own file with more detailed prompts.
...
## 6. Technical Implementation (with Google AI Studio)

*   **Google AI Studio's Role:** We will use Google AI Studio to:
    *   Generate the narrative text for each puzzle.
    *   Create the specific content for each puzzle (e.g., the pseudo-code poem, the JSON data).
    *   Define the personality of the "digital spirit."
*   **Hosting:** While the *content* is generated by the AI, the app itself (the HTML, CSS, and JavaScript) will be written and hosted separately. This project folder will contain the plan and prompts, not the final, running application.

## 7. Git Operations (User Handled Locally)

All Git operations, including cloning the repository, committing changes, and pushing to the remote, will be handled by the user locally. The Gemini CLI should not attempt to execute any `git` commands (e.g., `git clone`, `git commit`, `git push`) as this project uses a private repository and the CLI does not have the necessary authentication. The CLI's role will be to assist with file creation, modification, and content generation within the local project directory.

This document will serve as the "brain" for the project, ensuring that the AI in Google AI Studio has all the context it needs to help build this unique gift.
